...
const makeBlog = (db) => {
  const blogIndex = Math.random() * blogNames.length
  let fuzzedBlogIndex = fuzzCount(blogIndex)
  fuzzedBlogIndex = fuzzedBlogIndex < 0 ? 0 : fuzzedBlogIndex
  return db.collections.get('blogs').prepareCreate(blog => {
    blog.name = blogNames[fuzzedBlogIndex] || blog.id
  })
}
